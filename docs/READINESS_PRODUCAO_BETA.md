...
| Afiliados           | 🟡 em andamento, 3 lojas em estágios distintos — ver §3.1 | Amazon ativo (`tag=suplescore-20`), fiscal enviado (questionário `Completo`, retenção `0.0%`, tela da conta mostra `Enviado`), aguardando revisão da Amazon (3–5 dias úteis); Mercado Livre ativo e com clique real registrado, mas o link oficial de afiliado (`meli.la`, deeplink curto gerado por produto no painel) não é compatível com o modelo atual de `affiliateBaseUrl` por loja — exige mecanismo por oferta, ainda não implementado; Netshoes/Rakuten **rejeitado permanentemente** pelo anunciante Netshoes (confirmado pelo suporte Rakuten) — não é mais afiliado viável, tratar como loja sem monetização afiliada |
...
Levantamento de 2026-09-16, atualizado em 2026-09-17 (Netshoes/Rakuten). Nenhum `affiliateBaseUrl` novo foi inventado ou configurado aqui — só o status de cada loja, conforme confirmado no painel de cada rede.

| Loja                   | Status técnico                                                                                                                                                                                                               | Pendência                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Próximo passo                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Quem resolve                                                                                                       |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **Amazon**             | Ativo — `Store.isAffiliate=true`, `affiliateBaseUrl="tag=suplescore-20"`, cliques saindo com `wasAffiliate=true`                                                                                                             | **Fiscal enviado, aguardando revisão da Amazon** — questionário fiscal preenchido/enviado no portal Amazon Associates, com status `Completo` e retenção `0.0%` dentro do questionário; a tela da conta mostra `Enviado`. Amazon pode levar de 3 a 5 dias úteis para revisar/confirmar a situação fiscal final (não é limitação técnica do SupleCheck)                                                                                                                                                                                                                                                                                                                                                                 | Aguardar a revisão da Amazon (3–5 dias úteis); nenhuma ação adicional necessária no portal por enquanto                                                                                                                                                                                                                                                                                                                                                                                                             | Amazon (prazo de revisão deles), fora do código                                                                    |
| **Mercado Livre**      | Conta ativa, cliques reais já sendo registrados (`OutboundClick`)                                                                                                                                                            | **Validado no painel** (botão "Compartilhar" em um produto real): o link oficial é um deeplink curto `https://meli.la/<código>` (ex.: `https://meli.la/16T3cTu`), gerado individualmente por produto — junto com um "ID para busca" alfanumérico (ex.: `BRJ2RV-5HX1`) para colar no buscador do próprio Mercado Livre. **Não é** a URL comum do produto nem uma querystring/wrapper aplicável a qualquer URL — não se encaixa no modelo atual de `Store.affiliateBaseUrl` (que pressupõe um template único por loja, aplicado a qualquer `PriceEntry.url`). Mercado Livre **não deve ser considerado 100% monetizado** só por estar `isAffiliate` ativo — o redirect atual usa a URL direta do produto, sem tracking. | **Não configurar `affiliateBaseUrl` genérico** — não existe um valor único que sirva para todos os produtos. Investigar primeiro se o painel do Mercado Livre oferece alguma ferramenta de gerar link afiliado a partir de uma URL de produto arbitrária (não só busca manual produto a produto); se não existir, tratar como exigindo um campo por-oferta (ex.: `PriceEntry.affiliateUrl` ou mecanismo equivalente) — mesma categoria de gap já registrada para programas por cupom (ver `BETA_OPERACIONAL.md §7`) | Próximo passo técnico/comercial desta frente — decisão de arquitetura (campo por-oferta), não configuração simples |
| **Netshoes / Rakuten** | **Rejeitado permanentemente** — confirmado pelo suporte da Rakuten Advertising: a conta foi rejeitada pelo anunciante Netshoes, e é essa rejeição (não um bug de painel) que explica os links não clicáveis observados antes | Sem programa de afiliado viável para Netshoes por enquanto — não é uma pendência técnica temporária, é uma decisão do anunciante                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Não configurar `affiliateBaseUrl` para Netshoes, não insistir no painel Rakuten/Netshoes; se houver produto apontando para Netshoes, tratar como loja sem monetização afiliada por enquanto, ou trocar futuramente por outra loja com afiliado válido                                                                                                                                                                                                                                                               | Decisão comercial (trocar de loja ou aceitar sem afiliado), fora do controle técnico do SupleCheck                 |
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Mercado Livre (link `meli.la` por produto não cabe no modelo atual) e Netshoes/Rakuten (rejeitado permanentemente pelo anunciante) ainda sem monetização real ativa (ver §3.1) | Médio      | Não é bug — Mercado Livre precisa de decisão de arquitetura (campo por-oferta, não `affiliateBaseUrl` por loja); Netshoes não tem mais caminho de afiliado viável, decisão é comercial (trocar de loja); `/admin/monetizacao` mostra isso honestamente |
