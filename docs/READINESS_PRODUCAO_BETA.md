...
| Imagens             | 🟡 pipeline pronto, publicação pendente de infraestrutura | Candidatos já aprovados na fila (`PendingImage`), botão "Publicar no Blob" funcional — só falta `BLOB_READ_WRITE_TOKEN` real (ver §2 e §4)                                                                                                                                                                                                           |
| Afiliados           | 🟡 infraestrutura pronta, ativação comercial pendente     | `configureStoreAffiliate.ts` com dry-run funcional, `wasAffiliate` gravado corretamente — só falta o link/tag real de cada programa aprovado (ver §2 e §5)                                                                                                                                                                                           |
...
- Pipeline de imagem (descoberta → candidato → aprovação → publicação no Blob) implementado e testado end-to-end contra um Blob Store de desenvolvimento — falta só o token de produção, não código.
...
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `BLOB_READ_WRITE_TOKEN` real                 | Publicação de imagem em produção (placeholders continuam no ar até lá — não é erro, é o fallback correto) | Conectar um Blob Store no painel da Vercel do projeto de produção                                                                                         | Vercel → projeto → Storage → Create Database → Blob (injeta a env sozinho)                                                       |
...
| ------------------------------------------------------------------------------------------ | ---------- | --------------------------------------------------------------------------------------------------------------------- |
| Nenhuma loja além de Amazon está com afiliado ativo                                        | Médio      | Não é bug — é o estado real até a aprovação comercial de cada programa; `/admin/monetizacao` mostra isso honestamente |
...
_Este documento reflete o estado real confirmado em auditoria até `58c3ced` (2026-09-16). Atualizar sempre que um insumo externo listado na §3 for resolvido (marcar como pronto em vez de pendente) ou que uma nova auditoria técnica mude o estado da §1._
