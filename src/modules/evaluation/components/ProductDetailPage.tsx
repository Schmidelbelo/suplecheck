...
export interface ProductDetailPageProps {
  /** Slug do produto (segmento final da URL). */
  slug: string;
  /**
   * Categoria esperada pela URL acessada — para `/creatina/[slug]` é
   * sempre `"creatina"`; para `/categorias/[slug]/[produto]` é o `[slug]`
   * da própria URL. Nunca a categoria REAL do produto até a validação
   * abaixo confirmar que as duas batem.
   */
  categorySlug: string;
}
...
/**
 * `generateMetadata` de `/creatina/[slug]` e de `/categorias/[slug]/[produto]`
 * chamam esta mesma função — nenhuma lógica de metadata duplicada entre
 * as duas rotas.
 */
export async function buildProductDetailMetadata({
  slug,
  categorySlug,
}: ProductDetailPageProps): Promise<Metadata> {
  const path = productDetailPath(categorySlug, slug);
  const view = await loadProduct(slug);
  if (!view || view.product.categorySlug !== categorySlug) {
    return buildMetadata({ title: "Produto não encontrado", path, noIndex: true });
  }
...
/**
 * Renderização compartilhada da página de detalhe de produto —
 * consumida por `/creatina/[slug]` (categoria com rota própria) e por
 * `/categorias/[slug]/[produto]` (qualquer outra categoria). Nenhuma
 * lógica de apresentação duplicada entre as duas rotas: só o
 * `categorySlug` muda.
 *
 * Validação de categoria: `view.product.categorySlug` (a categoria REAL
 * do produto, vinda do banco) precisa bater com `categorySlug` (o que a
 * URL diz) — caso contrário, 404 real. Isso cobre tanto slug inexistente
 * quanto a URL de uma categoria errada apontando para um produto de
 * outra (nunca renderiza um produto sob o rótulo/ranking de uma
 * categoria que não é a dele).
 */
    ...
    <>
      <JsonLd
        data={breadcrumbSchema([
          { label: "Home", href: "/" },
          { label: presentation?.categoryName ?? categorySlug, href: basePath },
          { label: product.name, href: productDetailPath(categorySlug, slug) },
        ])}
      />
      <JsonLd
        data={productSchema({
          ...
          <div className="flex flex-col gap-6">
            ...
            {ranking ? (
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  #{ranking.position} de {ranking.total} no ranking de{" "}
                  {presentation?.categoryName ?? categorySlug}
                </Link>
        ...
        <Section id="comparacao" className="border-border scroll-mt-20 border-b">
          <div className="mx-auto flex max-w-3xl flex-col gap-6">
            ...
            <p className="text-text-subtle text-xs">
              Média calculada sobre os {averages.count} produtos avaliados no ranking de{" "}
              {presentation?.categoryName ?? categorySlug}.
            </p>
          </div>
        </Section>
      ...
      <Section id="informacoes-tecnicas" className="scroll-mt-20">
        <div className="mx-auto flex max-w-3xl flex-col gap-6">
          ...
          >
            ...
            Ver todos os produtos do ranking de {presentation?.categoryName ?? categorySlug}
          </Link>
        </div>
      </Section>
