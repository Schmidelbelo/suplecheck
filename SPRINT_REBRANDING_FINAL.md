...
- `NEXT_PUBLIC_SITE_URL`: fallback padrão em `src/config/site.ts` atualizado para `https://suplescore.com.br`, deixado preparado para quando o DNS propagar.
...
- **Vercel**: a env var `NEXT_PUBLIC_SITE_URL` de produção (gerenciada no painel, fora do repositório) deve ser confirmada como `https://suplescore.com.br` depois do DNS confirmado, para não derrubar `/creatina`.
