...
/**
 * Sprint de Crescimento — 2º produto em Coenzima Q10. Growth Supplements
 * Coenzima Q10 100mg 60 Cápsulas — 100mg de CoQ10 por cápsula/dose,
 * confirmado de forma consistente em múltiplas fontes (Drogasil, Droga
 * Raia, Mercado Livre, Essência Brasileira). Preço real da Essência
 * Brasileira. Categoria "coenzima-q10" já ativa (metodologia já
 * configurada).
 */

const CATEGORY_SLUG = "coenzima-q10";

const PRODUCT = {
  slug: "growth-coenzima-q10-100mg-60-capsulas",
  name: "Growth Supplements Coenzima Q10 100mg 60 Cápsulas",
  variantLabel: "60 cápsulas",
  coq10PerDoseMg: 100,
  servingsPerUnit: 60, // 1 cápsula por dose
  ingredientsText: "Coenzima Q10 (ubiquinona), sem sabor, sem glúten, sem lactose.",
  sourceUrl:
    "https://www.essenciabrasileira.com.br/produto/growth-supplements-coenzima-q10-100mg-60-caps-sabor-sem-sabor/",
} as const;

const PRICE = {
  storeSlug: "essencia-brasileira",
  priceCents: 8230,
  url: "https://www.essenciabrasileira.com.br/produto/growth-supplements-coenzima-q10-100mg-60-caps-sabor-sem-sabor/",
  capturedAt: "2026-09-09",
} as const;

const REFERENCE_RANGE_MG = { min: 30, max: 200 } as const;
...
async function createProductAndSku() {
  ...
  const product = await prisma.product.upsert({
    ...
    create: {
      ...
      status: "DRAFT",
      attributes: {
        coq10PerDoseMg: PRODUCT.coq10PerDoseMg,
  ...
  if (!sku) {
    sku = await prisma.sku.create({
      data: {
        ...
        dosagePerServing: PRODUCT.coq10PerDoseMg,
        status: "ACTIVE",
      },
    });
  }
...
async function publishAndEvaluate() {
  ...
  await container.useCases.evaluateSupplement.execute({
    ...
    facts: {
      composition: {
        activeIngredientAmountPerServing: PRODUCT.coq10PerDoseMg,
