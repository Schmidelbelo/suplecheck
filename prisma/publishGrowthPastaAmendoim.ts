...
/**
 * Sprint de Crescimento — Pasta de Amendoim (categoria nova). Growth
 * Supplements Pasta de Amendoim Integral Torrado 1kg — composição
 * confirmada via OpenFoodFacts (base de dados nutricional aberta):
 * 4,1g de proteína por porção de 15g (rótulo). Preço real da Gold Star
 * Supplements. Nenhuma metodologia dedicada existe para Pasta de
 * Amendoim — reaproveita "creatina-methodology".
 */

const CATEGORY = { slug: "pasta-de-amendoim", name: "Pasta de Amendoim" } as const;

const PRODUCT = {
  slug: "growth-pasta-de-amendoim-integral-torrado-1kg",
  name: "Growth Supplements Pasta de Amendoim Integral Torrado 1kg",
  variantLabel: "1kg",
  proteinPerDoseGrams: 4.1,
  servingsPerUnit: 66, // 1000g / 15g por porção ≈ 66 porções
  ingredientsText:
    "Amendoim torrado (ingrediente único). 88kcal / 3g carboidratos por porção de 15g.",
  sourceUrl:
    "https://br.openfoodfacts.org/produto/7896029046173/pasta-de-amendoim-integral-growth-natural",
} as const;

const PRICE = {
  storeSlug: "goldstar-supplements",
  priceCents: 7998,
  url: "https://goldstarsupplements.com.br/produtos/pasta-de-amendoim-integral-torrado-1kg-original-growth-supplements/",
...
async function ensureCategoryAndStore() {
  ...
  await prisma.store.upsert({
    ...
    create: { slug: PRICE.storeSlug, name: "Gold Star Supplements", isAffiliate: false },
    update: {},
  });
}
